...
- 不物理删除 TaskItem。
- 不物理删除 TaskItemData。
- 只把指定 TaskItemData 的 `status` 改为 `删除`。
...
- 如果 `confirm_state = rejected`，返回参数错误。
- 否则设置 `remote_state = success`。
- 设置 `remote_at = now`。

注意：当前实现是提交状态占位，未看到真实调用远端更新接口。
...
```text
TaskItemData.status = 删除
```

两者都不会删除源媒体，也不会删除源 TaskItem。
...
| 接口 | 说明 |
|---|---|
| `GET /api/reviews/completed-tasks` | 查询已完成且有 TaskItem 的任务 |
| `GET /api/reviews?taskId=1` | 投影旧复核卡片 |
| `POST /api/reviews/confirm` | 委托 TaskItem 确认 |
| `DELETE /api/reviews/{id}` | 标记复核行删除 |
| `POST /api/reviews/delete` | 批量标记删除 |
...
```bash
curl -X POST http://localhost:8000/api/task-items/action-delete \
  -H "Content-Type: application/json" \
  -d '{"task_item_id": 1, "task_item_data_ids": [10, 11]}'
```
